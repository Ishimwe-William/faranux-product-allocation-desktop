...
export class Store {
    constructor() {
        this.state = {
            ...
            woocommerce: {
                products: [],
                loading: false,
                progress: {count: 0, total: 0}, // Added progress tracking
                error: null,
                lastSync: null
            },
            config: {
    ...
    setWooProducts(products) {
        this.setState({
            woocommerce: {
                products,
                loading: false,
                progress: {count: 0, total: 0}, // Reset on complete
                error: null,
                lastSync: new Date().toISOString()
            }
        });
    ...
    setWooLoading(loading) {
        const currentProgress = this.state.woocommerce.progress;
        this.setState({
            woocommerce: {
                ...this.state.woocommerce,
                loading,
                // Reset progress if starting new load
                progress: loading ? {count: 0, total: 0} : currentProgress
            }
        });
    }

    // New method to update progress
    setWooProgress(count, total = 0) {
        this.setState({
            woocommerce: {
                ...this.state.woocommerce,
